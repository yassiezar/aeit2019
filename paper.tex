...
~
  ...
  \caption{Pictures of the latest Tango mobile phone model (left), the Tango tablet and bone-conducting headphones that were used in this work (centre) and both in use in an experiment (right).}
\end{figure}
...
Electronic navigation aids and guidance systems for people with vision impairments often have acceptability and usability problems due to limitations in academic research and prototypes: prohibitive costs, unfriendly user interfaces, significant hardware requirements (e.g.\ head-mounted cameras and GPS antennae)~\cite{golledge2004stated,yusif2016older,arditi2013user}.
...
However, the participants with vision impairments in group \textit{G2}  demonstrate a wider spread in error data and higher average error than \textit{G1} (0.23 vs. 0.45 radians, Kruskal-Wallis test $p<0.001$). 
...
Interestingly, the elevation errors for group \textit{G2} are centred around the negative angle, as seen in \cref{fig:err-results}, indicating a possible underlying bias in the participants or the interface.
