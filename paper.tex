...
\institute{University of Lincoln \email{\{jlock, gcielniak, nbellotto\}@lincoln.ac.uk} \and
...
\begin{abstract}
  ...
  This interface was validated through a set of experiments with blindfolded and visually impaired participants. 
...
The UK's Royal National Institute of Blind People for example has prioritised improving access to everyday services and products, such as public transport and mobile apps~\cite{rnib2016uk}.
...
The ActiVis project aims to deliver a mobile guidance system that will ultimately be able to guide a user with vision impairments on the last leg of their journey, addressing the so-called `last 10-yards problem'. 
The prototype platform is based on a Google Project Tango\footnote{\url{https://en.wikipedia.org/wiki/Tango\_(platform)}} device, pictured in \cref{fig:tango} and \cref{fig:tango-headphone}, provides access to powerful real-time localisation (through IMU measurements and landmark tracking with embedded RGB-D camera) and image-processing facilities. 
...
~
  \subfloat[]{\label{fig:tango-headphone}\includegraphics[width=0.35\columnwidth]{figures/tango_headphone.png}}
~
...
Humans are able to determine the 3D position of a sound source and by exploiting this natural ability, the system's real-time guidance instructions can be interpreted without posing a significant cognitive load.
...
A similar approach was used with success in~\cite{durette2008visuo}, but the study did not focus on or investigate the interface's efficacy.
...
The paper starts by discussing previous relevant works and research in \cref{sec:prev-work}, followed by a discussion on the design and implementation of our interface in \cref{sec:interface-design}.
...
The majority of these systems rely on one or a combination of vocal~\cite{mocanu2016when,chessa2016integrated,kanwal2015navigation}, audio~\cite{schwarze2015intuitive,rodriguez2012obstacle,katz2010navig} and haptic~\cite{rivera-rubio2015assistive,lee2015rgb,xiao2015assistive} feedback media to communicate guidance instructions to the user, each with their own set of features and limitations.
In general, participants with visual impairments report that they prefer haptic and vocal feedback out of the three aforementioned options~\cite{arditi2013user}.
However, haptic feedback systems typically have extra hardware requirements to reliably transmit guidance signals.
...
Indeed, participants report that they would prefer control over the vocal feedback channel and manually trigger guidance instructions instead of being given constant guidance instructions~\cite{arditi2013user}.
Simple audio tones are less affected by bandwidth and hardware limitations, but they can potentially fatigue the user if they are too unpleasant.
...
However, these studies focussed on their system's total effectiveness, rather than the effectiveness of the pitch adjustment mechanism alone. 
In the next sections we investigate this more closely. 
...
The ActiVis system is based on a concept proposed in~\cite{lock2017portable,lock2019active} that uses a Google Tango device that is able to localise itself in real-time and has the benefit of a compact, familiar form-factor which will help overcome the hurdle of user-acceptance and usability.

A set of bone-conducting headphones is used as the audio transmission medium.
These headphones sit on a user's cheekbones and conduct the audio signals through the skull into the inner ear instead of through the outer part like typical over-ear headphones do. 
This has the benefit of giving the user access to ambient sounds and noise, so a person with limited vision can still rely on sound to detect, for example, oncoming vehicles and people~\cite{lichtenstein2012headphone}.
...
The AfterShockz headphones (\cref{fig:tango-headphone}) were ultimately selected since they do not interfere with any other sounds and are also more discreet than the larger over-ear headphones. 

\subsection{Human Audio Localisation}

Humans localise sound in 3 dimensions by considering cues recorded in one ear (monaural cues) and comparing cues received at both ears (binaural cues)~\cite{blauert1997spatial,blauert1969sound}.
Binaural cues include inter-aural time and level differences (ITD and ILD respectively) that help to determine a source's location on the horizontal plane, while monaural cues are taken from the interaction of the sound with the human anatomy, e.g.\ head, shoulders, outer ear, before it enters the ear canal.
...
When an audio signal is transmitted via a set of speakers or headphones, it can be transformed with an HRTF to mimic the characteristics of a natural sound source, tricking the brain into believing a sound is located at some arbitrary position.
An HRTF is a mathematical function that simulates the response signal of the human head and is derived by capturing key characteristics that affect the monaural and binaural responses, such as the user's hearing levels, ear shape and head size.
...
However, given the complicated process involved to capture the required user characteristics, making unique HRTFs is often an untenable solution and using average values (e.g.\ head measurements, height, etc.) produces acceptable results~\cite{gardner1995hrtf}.

\subsection{Interface Design}

Guidance information is presented to the user in terms of pan and elevation angles, indicating the angular adjustments required to point the device camera at the target location, as shown in \cref{fig:cam-coords}.
...
The pan angle, instead, can be conveyed by transforming the audio signal with an HRTF, and indeed it has been found that this dimension is unaffected by bone-conducting headphones~\cite{schonstein2008comparison,macdonald2006spatial,stanley2006lateralization}. 
...
In this work, a pure sinusoidal wave was used, which people typically have trouble localising given its lack of a sufficiently rich spectral signature.
...
To compensate for this, the target's elevation angle is conveyed by adjusting the tone's pitch (i.e.\ the sine wave's frequency) as a function of the elevation. 
When the camera vector is at the correct elevation, the tone pitch is set to neutral and when the target is above or below the camera vector, the pitch is increased or decreased, respectively.
...
We set the pitch limits to 2 octaves from the neutral pitch, giving frequency limits of [\SI{128}{\hertz}, \SI{2048}{\hertz}].
...
When the user taps the device screen, a new virtual target is generated and its coordinates are sent to the audio generation module, along with the device's current position and orientation.
...
To test the interface's effectiveness at guiding the user in a pointing task, a set of experiments were conducted to capture the difference between the targets' actual directions and the directions the participants perceived them to be.
The participants were given a Tango device running an app written for the experiment that generates a set of virtual targets and presents them to the participants one at a time. 
...
Group \textit{G1} consisted of 10 young adults with normal eyesight who were blindfolded for the experiments, and group \textit{G2} contained 4 legally blind participants. 
Both groups were given some time before the experiments to familiarise themselves with the system, the audio signal's behaviour and the \SI{512}{\hertz} on-level tone. 
...
The results in~\cite{katz2011spatial,zwiers2001spatial} reported a similar trend, although an investigation with a larger sample size for \textit{G2} should be considered before definitive conclusions can be drawn.
...
As with the pan estimation errors, the participants from \textit{G1} seem more adept at estimating the correct elevation.
...
Indeed, the performance is comparable to that of more expensive open-back and in-ear headphones.
...
However, all the participants in~\cite{stanley2006lateralization} had healthy eyesight, making the latter comparison less valid. 
Interestingly, the elevation errors for both groups are centred around a negative angle, as shown in \cref{fig:err-results}, indicating a possible underlying bias either in the participants or the interface.
A similar trend was observed in~\cite{stanley2006lateralization}. 
Further investigation with a larger participant pool is needed to determine this underlying bias, if any. 

The error data was used to generate scatter plots that show the correlation between the targets' actual positions and their positions as perceived by the participants.
...
In this work, we propose an audio interface that conveys a target's pan and elevation angles through a spatialised audio tone and bone-conducting headphones.
...
Furthermore, the limitations of the latter were largely overcome by communicating the target's elevation with the tone's pitch, achieving pan and elevation errors comparable to that of over-ear headphones.
