...
\begin{abstract}
  ...
  To this end, we propose a new audio-based interface that uses a spatialised signal to convey a target's direction on the horizontal plane. 
  The vertical direction on the median plan is given by adjusting the tone's pitch to overcome the audio localisation limitations of bone-conduction headphones. 
  This interface is validated through a set of experiments with blindfolded and visually impaired users. 
\keywords{Human-machine interface \and visual impairment \and spatialised sound \and varying pitch \and bone-conduction}
\end{abstract} 
...
The ActiVis project aims to deliver a mobile guidance system, pictured in \cref{fig:tango} and \cref{fig:tango-headphone}, that will ultimately be able to guide a user with vision impairments on the last leg of their journey, i.e.\ the so-called `last 10-yard problem'. 
...
~
  ...
  \caption{Pictures of the latest Tango mobile phone (left), the former Tango tablet and the bone-conducting headphones used in this work (centre), and a visually impaired user during an experiment (right).}
\end{figure}
...
Electronic navigation aids and guidance systems for people with visual impairments often have acceptability and usability problems due to limitations in academic research and prototypes: prohibitive costs, unfriendly user interfaces, significant hardware requirements (e.g.\ head-mounted cameras and GPS antennae)~\cite{golledge2004stated,yusif2016older,arditi2013user}.
...
The ActiVis system is based on a concept proposed in~\cite{lock2017portable,lock2019active}, using a Google Tango device that is able to localise itself in real-time and has the benefit of a compact, familiar form-factor, which will help to overcome the hurdle of user-acceptance and usability.
...
Alternative solutions, such as open-back headphones, were also considered. These allow ambient noise through, but they still filter the incoming sound. The bone-conduction AfterShockz headphones (\cref{fig:tango-headphone}) were ultimately selected since they do not interfere on other sounds and are also more discreet than the larger over-ear headphones. 
...
In this work, a pure \SI{10}{\hertz} sinusoidal wave was used. 
...
When the camera vector is at the right target's elevation, the tone pitch is set to neutral.
...
Interestingly, the elevation errors for group \textit{G2} are centred around the negative angle, as shown in \cref{fig:err-results}, indicating a possible underlying bias either in the participants or the interface.
